...
from app.services.template_load_generator import TemplateLoadGenerator
...
async def generate(number: int, background_tasks: BackgroundTasks):
    try:
        background_tasks.add_task(TemplateLoadGenerator.insert_templates, number)
        response = JSONResponse(
            status_code=200,
            content={
                "success": "true",
        ...
        logger.error(f"Error in router generating entries: {e}")
        response = JSONResponse(
            status_code=400,
            content={
                "success": "false",
                "message": "Error generating entries",
                "details": str(e)
            }
        )
    return response
